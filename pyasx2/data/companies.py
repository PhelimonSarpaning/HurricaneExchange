...
def get_listed_companies():
    ...
    try:
        ...
        response = requests.get(pyasx2.config.get('asx_companies_csv'), stream=True)
        response.raise_for_status()  # throw exception for bad status codes

    except requests.exceptions.HTTPError as ex:

        raise pyasx2.data.LookupError("Failed to lookup listed companies; %s" % str(ex))
...
def _normalise_share_dividend_info(raw):
    ...
    if 'last_dividend' in raw:
        ...
        last_dividend['created_date'] = pyasx2.data._parse_datetime(last_dividend['created_date'])
        last_dividend['ex_date'] = pyasx2.data._parse_datetime(last_dividend['ex_date'])
        last_dividend['payable_date'] = pyasx2.data._parse_datetime(last_dividend['payable_date'])
        last_dividend['books_close_date'] = pyasx2.data._parse_datetime(last_dividend['books_close_date'])
...
def _normalise_company_info(raw):
    ...
    # parse dates to datetime objects
    company_info['listing_date'] = pyasx2.data._parse_datetime(company_info['listing_date'])
    company_info['delisting_date'] = pyasx2.data._parse_datetime(company_info['delisting_date'])
...
def get_company_info(ticker):
    ...
    `pyasx2.data.securities.get_security_info()`.
    ...
    endpoint_pattern = pyasx2.config.get('asx_company_json')
    ...
    if 'primary_share' in raw_info:
        share_info = pyasx2.data.securities._normalise_security_info(raw_info['primary_share'])
    else:
        share_info = pyasx2.data.securities.get_security_info(ticker)
...
def _normalise_annoucements(raw_annoucements):
    ...
    if 'data' in raw_annoucements:
        ...
        for raw_annoucement in raw_annoucements['data']:
            ...
            # parse dates to datetime objects
            annoucement['document_date'] = pyasx2.data._parse_datetime(annoucement['document_date'])
            annoucement['release_date'] = pyasx2.data._parse_datetime(annoucement['release_date'])
...
def get_company_announcements(ticker):
    ...
    endpoint_pattern = pyasx2.config.get('asx_announcements_json')
    endpoint = endpoint_pattern % ticker.upper()
